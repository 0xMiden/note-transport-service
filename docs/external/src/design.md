...
The node supports PostgreSQL and SQLite with explicit schema migration. The serving process checks migration versions and checksums without changing the schema. In-memory databases are reserved for tests.
